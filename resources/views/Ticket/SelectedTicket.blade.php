<x-app-layout>
        ...
        <div class="grid md:grid-cols-2">
            <div class="flex justify-start">
                <div class="inline-block mr-20">
                    <a href="{{ route('ShowListTickets', ['category' => $category]) }}">Back</a>
                </div>
                <div>
                    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                        Observation #{{ $ticket->ticket_id }} <span
                            class="inline-block @if ($ticket->status == 'In Progress') bg-yellow-200 @elseif($ticket->status == 'Declined') bg-red-200 @else bg-green-200 @endif rounded-full py-1 px-3">{{ $ticket->status }}</span>
                    </h2>
                </div>
            </div>
            <div class="text-right">
                @if ($isShowButton)
                    <button class="bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-3 rounded"
                        onclick="handleClickActionButton(true, '{{ $approveButtonText }}')">{{ $approveButtonText }}</button>
                    <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-3 rounded"
                        onclick="handleClickActionButton(true, 'Declined')">Decline</button>
                @endif
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 md:mb-5">
            ...
            <div class="py-3">
                <h4 class="text-xl">Before Action Taken</h4>
            </div>
                            ...
                            <div class="flex flex-col gap-y-8">
                                <div class="grid md:grid-cols-2 gap-x-3">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-red">
                                            <strong>{{ \Carbon\Carbon::parse($ticket->dateline)->format('d/m/Y') }}</strong>
                                        </div>
                                    </div>
                                </div>
                                <div class="grid md:grid-cols-2 gap-x-3">
                                    ...
                                    <div>
                                        ...
                                        <ul class="list-disc">
                                            @if ($ticket->plant_involve)
                                                <li>
                                                    <div>{{ $ticket->plant_involve?->head_plant?->name ?? 'No item' }}</div>
                                                    <div>{{ $ticket->plant_involve?->head_plant?->email ?? 'No item' }}
                                                    </div>
                                                </li>
                                            @else
                                                <div>No item</div>
                                        ...
                                        <ul class="list-disc">
                                            @if ($ticket->sub_dept_res_id == 0)
                                                <li>
                                                    <div>
                                                        {{ $ticket->dep_responsible?->head_department?->name ?? 'No item' }}
                                                    </div>
                                                    <div>
                                                        {{ $ticket->dep_responsible?->head_department?->email ?? 'No item' }}
                                                    </div>
                                                </li>
                                            @elseif ($ticket->sub_dept_res_id != 0)
                                                <li>
                                                    <div>
                                                        {{ $ticket->sub_dep_responsible?->head_subdepartment?->name ?? 'No item' }}
                                                    </div>
                                                    <div>
                                                        {{ $ticket->sub_dep_responsible?->head_subdepartment?->email ?? 'No item' }}
                                                    </div>
                                                </li>
                                            @else
                                                <div>No item</div>
