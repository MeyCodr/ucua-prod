<x-app-layout>
    ...
    <div class="py-6">
        <form action="{{ route('StateBranch.store') }}" method="post" enctype="multipart/form-data">
            @csrf
            <input type="text" name="formMode" value="{{ $formMode }}" hidden>
                        ...
                        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mt-2">
                            <div class="p-6 bg-white border-b border-gray-200">
                                <div>
                                    ...
                                    <div class="grid md:grid-cols-3 py-2">
                                        {{-- PIC 1 --}}
                                        <label for="pic1" class="col-span-1 self-center">Person in Charge:
                                            <span class="text-red-600">*</span></label>
                                        <div class="col-span-2">
                                            <select class="w-full rounded" id="pic1" name="pic1" required>
                                                <option value="" selected disabled>-- Please select one user --
                                                </option>
                                                @forelse ($pic_branches as $item)
                                                    <option value="{{ $item->id }}"
                                                        {{ old('pic1') == $item->id ? 'selected' : '' }}>
                                                        {{ $item->name }} -
                                                        {{ $item->email }}</option>
                                                @empty
                                                    <option>1</option>
                                                ...
                                                <div class="form-check mb-2">
                                                    <label class="form-check-label">
                                                        <input type="radio" class="form-check-input" value="1"
                                                            ...
                                                            @if (old('is_enabled') == 1) checked @endif>&nbsp;Active
                                                    </label>
                                                </div>
                                                <div class="form-check mb-2">
                                                    <label class="form-check-label">
                                                        <input type="radio" class="form-check-input" value="0"
                                                            ...
                                                            @if (old('is_enabled') == 0) checked @endif>&nbsp;Not
                                                        Active
                                                    </label>
                                                </div>
                                                @error('is_enabled')
